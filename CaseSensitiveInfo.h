...
#define FileCaseSensitiveInfo 71 // This value is typically 71
...
static const char *win_strerror(DWORD winerr) {
    if (!FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL,
                       ...
                       (LPWSTR)__win_strerror_buf, sizeof(__win_strerror_buf), NULL)) {
#ifndef NDEBUG
...
static int EnsurePathAccess(const char *path, DWORD access) {
    ...
    do {
        ...
        if ((winerr = GetNamedSecurityInfo((LPCWSTR)path, SE_FILE_OBJECT,
        ...
        BuildExplicitAccessWithName(&ea, (LPWSTR)userBuff, access, GRANT_ACCESS, SUB_CONTAINERS_AND_OBJECTS_INHERIT);
        ...
        if ((winerr = SetNamedSecurityInfo((LPWSTR)pathBuff, SE_FILE_OBJECT, DACL_SECURITY_INFORMATION, NULL, NULL, newDacl,
                                           NULL))) {
            SetLastError(winerr);
...
__attribute__((constructor)) static void __init_creds(void) {
    ...
    if (!GetUserName((LPWSTR)userBuff, &dw)) {
...
static int __open_dir_fd(const char *path, DWORD access, DWORD share_mode, int flags) {
    ...
    if ((h = CreateFile((LPCWSTR)path, access, share_mode, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL)) ==
