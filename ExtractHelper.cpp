...
namespace skkk {
...
#else
        struct utimbuf ub = {};
        ub.actime = static_cast<time_t>(inode->i_mtime);
        ub.modtime = static_cast<time_t>(inode->i_mtime);
    ...
    int writeErofsNode2File(ErofsNode *eNode, const string &outDir) {
        ...
        if (hardlinkSrcPath) {
            std::unique_lock<std::mutex> lock(erofsHardlinkLock);
